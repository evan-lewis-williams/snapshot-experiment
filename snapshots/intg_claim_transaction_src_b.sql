{% snapshot intg_claim_transaction_src_b %}
...
WITH source AS (
select distinct concat(ts.id, '~', t.id, '~', tl.id,'~','B') as claim_transaction_key
    ,'A' as src
...
from {{ ref('source_cc_transaction_set_src_b') }} ts 
  left join {{ ref('source_cc_transaction_src_b') }} t on ts.id = t.transetid
  left join {{ ref('source_cc_transaction_line_src_b') }} tl on t.id = tl.tranid
